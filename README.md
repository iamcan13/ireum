...
즐겨찾기/공유를 여러 기기에서 동기화하려면 `.env.example`을 참고해 Supabase 프로젝트를 만들고,
`src/lib/storage/index.ts`의 `getStorage()`가 Supabase 어댑터를 반환하도록 구현하세요. **UI 코드는 그대로**입니다.
...
src/
  ...
  lib/
    ...
    hanja/        # 작명 한자 풀
    core/         # 오행 상수
    storage/      # 영속성 추상화(localStorage · Supabase-ready)
